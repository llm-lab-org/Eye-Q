...
class QwenAPI(VisionLanguageModel):
    def __init__(
        self,
        ...
        self.api_key = api_key or os.getenv("QWEN_API_KEY") or os.getenv("OPENROUTER_API_KEY")
        if not self.api_key:
            raise RuntimeError("Missing QWEN_API_KEY (or OPENROUTER_API_KEY)")
        self.model_name = model or os.getenv("QWEN_MODEL") or "qwen/qwen3-vl-235b-a22b-instruct"
        self.base_url = (base_url or os.getenv("QWEN_BASE_URL") or "https://openrouter.ai/api/v1").rstrip("/")
    ...
    def generate_chat(
        ...
        if resp.status_code != 200:
            raise RuntimeError(f"Qwen/OpenRouter error {resp.status_code}: {resp.text}")
        data = resp.json()
