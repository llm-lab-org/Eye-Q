...
class LlamaAPI(VisionLanguageModel):
    def __init__(
        self,
        ...
        self.api_key = api_key or os.getenv("LLAMA_API_KEY") or os.getenv("AVALAI_API_KEY")
        if not self.api_key:
            raise RuntimeError("Missing LLAMA_API_KEY (or AVALAI_API_KEY)")
        self.model_name = model or os.getenv("LLAMA_MODEL") or "llama-4-scout-17b-16e-instruct"
        self.base_url = (base_url or os.getenv("LLAMA_BASE_URL") or os.getenv("AVALAI_BASE_URL") or "https://api.avalai.ir/v1").rstrip("/")
    ...
    @property
    def name(self) -> str:
        return f"llama-{self.model_name}"
    ...
    def generate_chat(
        ...
        for msg in messages:
            ...
            if msg.get("image_path"):
                parts.append({"type": "image_url", "image_url": {"url": _image_to_data_url(msg["image_path"])}})
        ...
        if resp.status_code != 200:
            raise RuntimeError(f"Llama/OpenAI-compatible error {resp.status_code}: {resp.text}")
        data = resp.json()
